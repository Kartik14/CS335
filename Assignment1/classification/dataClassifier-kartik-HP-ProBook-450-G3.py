...
def enhancedFeatureExtractorDigit(datum):
    ...
    delta = [1,-1]
    for x in range(1,DATUM_HEIGHT-1):
        for y in range(1,DATUM_WIDTH-1):
            temp = True
            for dx in delta:
            ...
            for dy in delta:
                if getPixelVal(x,y) != getPixelVal(x,y+dy):
                    temp = False
            if getPixelVal(x,y) and temp:
                features[1] += 1
    
    for x in range(DATUM_HEIGHT):
        for y in range(DATUM_WIDTH-1):
            if abs(getPixelVal(x,y+1) - getPixelVal(x,y)) == 1:
                features[2] += 1

    for y in range(DATUM_WIDTH):
        for x in range(DATUM_HEIGHT-1):
            if abs(getPixelVal(x+1,y) - getPixelVal(x,y)) == 1:
                features[2] += 1
