...
def enhancedFeatureExtractorDigit(datum):
	...
	I am using 4 features which are described as follows:
	1) feature[0] -> This feature is just the area of the shape in the image.
	2) feature[1] -> This feature extracts the total number of "internal" black pixels(i.e. all the pixels in the 4-neighbourhood 
	of that pixel are also black) in the image.
	3) feature[2] -> This feature is the a measure of the perimeter of the shape in the image. This is calculated using the number
	of boundry pixels in the vertical and horizontal direction in the image.
	4) feature[3] -> This pixels is the number of black pixels in the top-right quarter of the image. This pixels was specifically
	meant to distinguish the square from the traingle (since the first three features were classifying all except square which 
	getting classified as triangels)
    ...
    delta = [-1,1]
    for x in range(DATUM_HEIGHT):
        for y in range(DATUM_WIDTH):
            if getPixelVal(x,y):
                temp = True
                for dx in delta:
                    if getPixelVal(x,y) != getPixelVal(x+dx,y):
                        temp = False
                for dy in delta:
                    if getPixelVal(x,y) != getPixelVal(x, y+dy):
                        temp = False
                if temp:
                    features[1] += 1

    for x in range(DATUM_HEIGHT):
        for y in range(DATUM_WIDTH-1):
            if abs(getPixelVal(x,y) - getPixelVal(x, y+1)) == 1:
                features[2] += 1

    for y in range(DATUM_WIDTH):
        for x in range(DATUM_HEIGHT-1):
            if abs(getPixelVal(x,y) - getPixelVal(x+1, y)) == 1:
                features[2] += 1

    for x in range(DATUM_HEIGHT/2):
        for y in range(DATUM_WIDTH/2):
            features[3] += getPixelVal(x,y)            
    
    features[3] = 1.25*features[3]
