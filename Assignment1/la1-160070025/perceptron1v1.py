		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				for label_ind in range(len(self.legalLabels)):
					label = self.legalLabels[label_ind]
					if label == trainingLabels[i]:
						continue
					else:
						temp = self.weights[frozenset([label, trainingLabels[i]])] * trainingData[i]
						if temp > 0:
							guess = max(label, trainingLabels[i])
						else:
							guess = min(label, trainingLabels[i])
						if guess != trainingLabels[i]:
							if(trainingLabels[i] < label):
								self.weights[frozenset([label, trainingLabels[i]])] -= trainingData[i]
							else:
								self.weights[frozenset([label, trainingLabels[i]])] += trainingData[i]
